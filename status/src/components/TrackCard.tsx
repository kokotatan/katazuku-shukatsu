  ...
  return (
    <article className="app-panel">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs text-slate-500">{track.priority || '通常'}</p>
        ...
        <span className={`rounded px-2 py-1 text-xs font-bold ${outcomeClass(track.outcome)}`}>{track.outcome || '進行中'}</span>
      </div>
