...
# meeting-autopilot が録音開始と同時に隠しプロセスとして起動する(tasks/people-ui-and-face-capture.md §2)。
# 撮った png は interview-digest が面接官の顔写真を切り出す元データになる。
#
# 方針(会議進行を絶対に妨げない):
...
foreach ($delay in ($delays | Sort-Object)) {
  ...
  # 撮影時刻まで分割待ち(record-audio.ps1 と同じ方式)
  while ($true) {
