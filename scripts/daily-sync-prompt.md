...
3. メールから企業ごとに以下を抽出する:
   ...
   - 次にやること → nextAction
   - 業界が分かれば industry
4. 抽出結果を `{name, stage, nextAction, nextDate, industry}` の配列JSONとして `status/sheet-import-loop.json` に書き出す(gitignore済み)。対象メールが無ければ「新しい選考情報はありませんでした」と出力して終了。
5. `cd status; npx tsx scripts/sheet-sync.ts sheet-import-loop.json` で dry-run 実行し、差分を確認する。
