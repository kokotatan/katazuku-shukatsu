...
9. 活動ログに1行残す(本人が「何を・何のために・どうしたか」を後から確認できる状態にするため)。実際の結果を How/Result に入れて実行する:
   `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/log-activity.ps1 -By daily-sync -Action "毎朝の選考同期" -Why "Gmailの新着から選考状況を最新化し取りこぼしを防ぐため" -How "<更新した企業名・件数、既読化/ゴミ箱の件数を簡潔に>" -Link "選考管理（新）タブ" -Result "<成功 / 鍵なしでシート同期はスキップ 等>"`
   (更新も既読化も何も無かった日は Action を「毎朝の選考同期(変化なし)」にする)。
10. 最後に結果サマリ(鍵の有無・シート同期の実行可否・更新した企業名・選考日・追記件数・既読化/ゴミ箱の件数)を簡潔に出力する。
