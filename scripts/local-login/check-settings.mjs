...
try {
  ...
  await writeFile(portalFile, JSON.stringify({ portals: {
    sso: { label: '合成SSO', loginUrl: 'https://sso.example.com/login', authMode: 'sso', note: '公開状態に含めないメモ' },
    password: { label: '合成採用サービス', loginUrl: 'https://ats.example.com/team/login' },
  } }))
  ...
  ok((await fetch(`${base}/api/local-login/session`, { headers: { Origin: 'https://outside.example.com' } })).status === 403, '外部originからトークンを取得できない')
  ...
  const reboundStatus = await new Promise((done, reject) => {
    const request = httpRequest(`${base}/api/local-login/session`, { headers: { Host: `outside.example.com:${server.address().port}` } }, (response) => {
      response.resume(); response.on('end', () => done(response.statusCode))
    })
    request.on('error', reject); request.end()
  })
  ok(reboundStatus === 403, 'DNS rebindingのHostを拒否する')
  ok((await post('settings', { preferences: prefs, revision: state.revision }, { Origin: 'https://outside.example.com' })).status === 403, '外部サイトから設定変更できない')
  ...
  ok((await post('portals/password/login', { url: 'https://outside.example.com' })).status === 400, 'GUIから任意URLを起動できない')
  ...
  const configure = { presetId: 'synth-preset', label: '', loginUrl: 'https://portal.example.com/login', authMode: 'sso', revision: state.revision }
  for (const loginUrl of ['', 'http://portal.example.com/', 'javascript:alert(1)', 'https://user:password@portal.example.com/', 'https://portal.example.com/?code=secret', 'https://portal.example.com/#secret']) {
  ...
  const changedResponse = await post('portals', { id: 'password', presetId: '', label: oldPortal.label, loginUrl: 'https://ats.example.com/another/login', authMode: 'password', revision: oldRevision })
  ...
  const firstAdded = await emptyService.configure({ presetId: 'synth-preset', label: '', loginUrl: 'https://login.example.com/', authMode: 'sso', revision: emptyState.revision })
  ...
  if (process.platform === 'win32') {
    ...
    const username = '合成ユーザー@example.com'
    ...
    ok(record.allowedPathPrefix === 'https://ats.example.com/team/', '共有ATSのテナントパスに資格情報を限定する')
