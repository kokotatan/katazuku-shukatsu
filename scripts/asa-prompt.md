...
- **番犬の死活(番犬の番犬)**: `logs/watchdog-heartbeat.txt` の時刻を見る。今から**8時間以上**古ければ、
  番犬(watchdog)自体が止まっている可能性として**【自動化の故障】**に含めて報告する。番犬が止まると
...
- 今日〜48時間以内にカレンダー上で面接・面談・座談会があれば、その企業のprepパックを作る:
  ...
  4. 逆質問案3つ
  5. 話し方の心構え(本人指定の固定文言 2026-07-29。毎回そのまま載せる):
     「愛嬌/笑顔/ハキハキ/分かりやすく/クリティカルに話す」
  6. 面接で話す核(本人指定の固定文言 2026-07-29。毎回そのまま載せる):
     「ガクチカの学び: 問いの設定が一番価値がある(マッチングプラットフォームから、非エンジニアでも
     自動化環境を作り上げられる制御ソフトウェアへのピボット)」
     「就活の軸: 市場(市場規模/人類の発展に寄与するドメイン/開かれる市場)・
