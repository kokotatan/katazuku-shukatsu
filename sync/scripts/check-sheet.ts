/**
 * シート書き戻し(src/sheet.ts)の動作チェック。
 * 実シートの構造(前置き行・2段ヘッダ・数式列・空きテンプレート行)を模して検証する。
 * 実行: cd sync && npx tsx scripts/check-sheet.ts
 */
import { desiredStatus, locateTable, planUpdates, sameCompany, type CellUpdate, type Company, type Stage } from '../src/sheet'
...
// 旧シートと同じ列構成: 企業名/業界/志望度/出願状況/選考①〜④/テスト種別/次回アクション/〆切・選考日/残り日数/提出済/…/メモ欄
...
const co = (name: string, stage: Stage, p: Partial<Company> = {}): Company => ({
  name, stage, nextAction: '', nextDate: null, ...p,
})

// --- sameCompany(名寄せ) ---
check('株式会社の有無は同一視', sameCompany('株式会社ネクストビート', 'ネクストビート'))
check('全角半角カッコは同一視', sameCompany('ギフティ（Giftee）', 'ギフティ(Giftee)'))
...
const HEADER2 = ['企業名', '時期', 'ポジション', '志望度', 'ステータス', '選考①', '選考②', '選考③', '選考④', '次アクション', '締切・選考日', '残り日数', '提出済', 'ES・資料URL', '選考メモ']
...
check('新スキーマ: ステータス/次アクション/締切列を検出', table2.cols.status === 4 && table2.cols.nextAction === 9 && table2.cols.nextDate === 10)
...
check('新: 手入力の次アクションは上書きしない', !at2(1, 9))
check('新: 空欄ステータスは補完する', at2(3, 4)?.value === '出願済')
check('新: 無い企業を空き行に追記', at2(4, 0)?.value === '新規ソフト社' && at2(4, 4)?.value === '出願済')
check('新: 残り日数・提出済(数式/チェック)に触れない', plan2.updates.every((u) => u.col !== 11 && u.col !== 12))
