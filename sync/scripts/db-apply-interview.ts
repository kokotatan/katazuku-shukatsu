...
import { ensureCareerSupportSchema, upsertCareerOrganization } from '../src/career-support'
...
export function applyInterview(input: InterviewInput, targetDb?: DatabaseSync): { created: boolean; interviewId: number; photos: number } {
  const db = targetDb ?? openDb(DB_PATH)
  ...
  return transaction(db, () => {
    ...
    for (const person of input.people || []) {
      // 「姓不明」等を実在人物として登録すると、別の面談の匿名担当者まで同一人物へ
      // 誤名寄せされる。氏名が判明するまでは議事録本文だけに残す。
    ...
    if (input.appointmentId) {
      ...
      db.prepare("UPDATE career_meeting SET status = 'completed', updated_at = ? WHERE id = ?")
        .run(now, input.careerMeetingId)
      db.prepare(`
