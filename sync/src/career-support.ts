...
import { normalizeAppointmentAt } from './db'
import { isAutomaticRecordingEligible } from './recording-eligibility'
...
export function ensureCareerSupportSchema(db: DatabaseSync): void {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS career_meeting (
      ...
      recordable INTEGER NOT NULL DEFAULT 0 CHECK (recordable IN (0,1)),
  ...
  addColumn('interview_note', 'organization_id', 'INTEGER REFERENCES career_organization(id)')
  addColumn('interview_note', 'career_meeting_id', 'INTEGER REFERENCES career_meeting(id)')
  addColumn('person', 'organization_id', 'INTEGER REFERENCES career_organization(id)')
}
