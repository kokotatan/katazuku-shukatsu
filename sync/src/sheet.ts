/**
 * 選考管理シート(Googleスプレッドシート)への書き戻しエンジン。
 * 旧 status/src/lib/sheet.ts から独立(2026-07-18 アプリ統合時に移設)。
 * 正本はシート。ここは daily-sync が Gmail から抽出した差分をシートに反映するためだけにある。
 *
 * 安全ルール:
 * - 合格/不合格/辞退 などの最終状態は上書きしない(合格の自動集計が壊れるため)
 * - 新スキーマ(ステータス列が自由記述)のタブでは、手入力を潰さないため空欄補完と新規追記のみ
...
export function sameCompany(a: string, b: string): boolean {
  ...
  // 短い名前(Go・トヨタ等)は部分一致だと誤マージするので完全一致のみ(両方4文字以上でだけ部分一致)
